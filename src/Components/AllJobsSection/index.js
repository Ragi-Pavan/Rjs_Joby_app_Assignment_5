...
class AllJobsSection extends Component {
  ...
  employFilter = event => {
    if (event.target.checked) {
      const value = CheckBoxList.includes(event.target.value)
      if (value !== true) {
        CheckBoxList.unshift(event.target.value)
      }
    } else {
      const index = CheckBoxList.indexOf(event.target.value)
      CheckBoxList.splice(index, 1)
    }
    this.setState({checkboxlist: CheckBoxList}, this.getJobsData)
  }
  ...
  getJobsData = async () => {
    ...
    const {checkboxlist, salaryRange, searchInput} = this.state
    const employOptions = checkboxlist.join(',')
